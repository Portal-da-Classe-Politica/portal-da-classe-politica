...
export const LastElectionMap = ({
  ...
  useEffect(() => {
    if (votesByState && mapElement.current && !mapRef.current) {
      let electionCityFeature: Feature | null = null;
      ...
      for (const stateVotes of Object.values(votesByState)) {
        ...
        featureLayers.push(citiesLayer);
      }
      ...
      mapRef.current.once('loadend', () => {
        const geometry = electionCityFeature?.getGeometry();
        if (geometry) {
          mapRef.current?.getView().fit(geometry.getExtent(), { padding: [20, 20, 20, 20], duration: 1500 });
        }
      });
